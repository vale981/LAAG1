...
\begin{definition}
  Sei $X$ eine Menge. Eine bijektion $\sigmaq: X \mapsto X$ heisst Permutation
  von X.
\end{definition}
...
\item
 ...
 \begin{proof}
   W"ahle eine Bais $B=(b_1,...,b_n)\in V$, definiere $\omega (v_1,...,v_n)$
   durch die Leibnitz Formel mit $\omega (b_1,..., b_n):=1$, dann ist $\omega
   \not= 0$ nach Konstruktion.
   \begin{trivlist}
...
Die Wahl einer Basis $B\subset V$ gibt uns eine Matrix $M^B_B(f)$. Wie findet
man eine Basis B, so dass $M^B_B(f)$ besonders einfach ist.

Gegebe eine Matrix $A\in K^{n\times n}$, finde eine invertierbare matrix S, s.d.
$S^{-1}\cdot A\cdot S$ besonders einfache form hat. (Die Suche nach $S$ ist die
Suche nach $M^{B}_{B'}(id)$)

\begin{exa}
  Sei $V$ ein $\mathbb{R}$ - Vektorraum, sei $B$ eine Basis. mit ...
\end{exa}


\begin{definition}
  Sei $f: V\mapsto V$ eine lineare Abbildung. Ein Eigenvektor von $f$ ist ein
  Vektor von $f$ist ein Vektor, so dass $f(v)=\lambda \cdot v$ f"ur ein ...
  $\lambda$ heisst eigenwert. $v$ nennt man den Eigenvektor zum eigenwert
  $\lambda$.
\end{definition}

Geometrisch: $f$ streckt $v$ mit dem koeffzienten $\lambda$.

\begin{lemma}
  Sei $f: V\mapsto V$ ein endomorphismus. Wenn .... eine Basis in $V$ aus
  Eigenvektoren mit den Eigenwerten ... ist (Eigenbasis), so gilt:
\end{lemma}

\begin{proof}
  Es gilt laut definition $f(b_i)=\lambda_i\cdot b_i,\; i=1,n$. Die Form von ...
  folgt aus Definition.
\end{proof}

Es folgt: wenn wir die Diagonalform von ... erreichen m"ochten, m"ussen wir
Eigenwerte und Eigenvektoren suchen (u. geg. finden!)

\begin{proof}
  Ein Endomorphismus $f$ heisst diagonalisierbar, wenn er eine Eigenbasis hat.
\end{proof}

\begin{proposition}
  Sei ... $\lambda \in K$
  \begin{enumerate}
  \item $v\in V$ ist ein Eigenvektor von $f$ zum Eigenwenwert ....
  \item $\lambda$ ist ein Eigenwerrt von f .... 
  \end{enumerate}
\end{proposition}

\begin{proof}
  \begin{enumerate}
  \item Sei $v\in V$ Eigenvektor zum Eigenwert $\lambda$. Dann gilt nach
    Definition $f(v)=\lambda\cdot v$ ...
  \item Nach (1) ist $\lambda$ ein Eigenwert $\iff$  ... 
  \end{enumerate}
\end{proof}
\begin{relation}
  \begin{enumerate}
  \item F"ur Suche nach Eigenvektoren bedeutet das: Wenn wir wissen, dass
    $\lambda$ Eigenwert ist, k"onnen wir Eigenvektoren durch die Bestimmung
    einer Basis im ... bestimmen. (L"osung von LGS)
  \item Um eigenwerte zu finden, m"ussen wir die Gleichung ... nach $\lambda$ l"osen.
  \end{enumerate}
\end{relation}

\begin{definition}
  Sei $f$ ... Das Polynom ... heisst charakteristisches Polynom von $f$. Die
  Gleichung ... Heisst charakteristische Gleichung von $f$. Aus den obigen
  "uberlegungen folgt: die Nullstellen von ... sind genau die Eigenwerte von $f$.
\end{definition}

Wenn wir also $f$ diagonalisieren wollen, m"ussen wir folgendes tun:
\begin{relation}
  \begin{enumerate}
  \item Charakteristische Gleichung l"osen: Eigenwerte bestimmen
  \item Finde Basen von $Ker(\lambda \cdot id - f)$ zu jedem Eigenwert
    $\lambda$.
...
\begin{definition}
  ... heisst Eigenraum zum Eigenwert $\lambda$
\end{definition}

\begin{proof}[Ausf"uhrlicher Beweis des Lemmas]
  Seien $v_1, ..., v_n$ Eigenvektoren zu Eigenwerten $\lambda_1,...,\lambda_n$
  (paarweise verschieden). Sei $... = 0$  Wende ... an.
\end{proof}

Was sind m"ogliche Hindernisse zur diagonalisierbarkeit?
\begin{enumerate}
\item Zu wenig Eigenwerte:
  \begin{exa}
    Rotation in $\mathbb{R}^2$ um $\frac{\pi}{2}$ .... Keine reellen
    Nullstellen. Hier schweitern wir daran, dass das Polynom $\lambda^2 + 1$
    nicht genug Nullstellen in $\mathbb{R}$ hat.
  \end{exa}
\end{enumerate}

\subsubsection{Nullstellen von Polynomen}
\label{sec:polyns}

\begin{definition}
  Sei $p\in K\[\lambda\]=\{\}$.... ein Polynom. Eine Nullstelle $\mu$ von $p$
  ist ein Element $\mu \in K$ so dass $p(\mu)=0$
\end{definition}

\begin{lemma}[Bezont]
  $\mu \in K$ ist eine Nullstelle vo $p \iff p(\lambda) = (\lambda -  \mu) \cdot
  p_1(\lambda)$
\end{lemma}

\begin{proof}[Skizze]
  \begin{itemize}
  \item[$\impliedby$] Offenbar
  \item[$\implies$] Folgt durch Polynomdivision.
  \end{itemize}
\end{proof}

\begin{folgerung}
  \begin{itemize}
  \item Ein Polynom von Grad $d$ h"ochstens $d$ Nullstellen.
  \end{itemize}
\end{folgerung}

\begin{theorem}[Fundamentalsatz der Algebra]
  Jedes Polynom $p\in \mathb{C}$ zerf"allt in Linearfaktoren. \[p(\lambda)= a_d
    (\lambda-\lambda_1)\cdot (\lambda -\lambda_2) ... (\lambda - \lambda_n)\]. ...
\end{theorem}
