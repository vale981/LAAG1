...
\begin{proof}[] \label{}
Definitionsgem"ass ist \(f: V\to W\) surjektiv genau dann, wenn \(lm(f)=W\).
\end{proof}
\end{exa}
...
\begin{definition}{}{}
Eine Bijektive Lineare Abbilfung \(f:V\to W\) hei"st Vektorraumisomorphismus
zwischen \(V\) und \(V\). \(V\) und \(W\) heissen isomorph, wenn es einen
Vektorraumisomorphismus \(f: V\to W\) gibt.
\end{definition}

\begin{exa}[] \label{}
Sei \(V\) ein Vektorraum, \(S\subseteq V = \{v_1, v_2, ..., v_n\}\). Die
Aufspannabbildung \(\varphi_S: \mathbb{K}^n \to V\) wird definiert als \(\varphi_S(\lambda) := \sum_{i=1}^n \lambda_iv_i \). Es gilt ("Ubung): \(\varphi_S\) bijektiv \(\iff S\) ist eine Basis.
\end{exa}

\textbf{Korrolar} \(S=\{v_1, \dots{}, v_n\}\) eine Basis \(\varphi_S: \mathbb{K}^n \xrightarrow{\cong} \) ein Isomorphismus (Bezeichnung \(\mathbb{K}^n \cong V\))

\textbf{Korrolar} \(\dim V = n \iff V\cong \mathbb{K}^n \). (\((\impliedby)\) isomorphe Vektorraume haben
die gleiche Dimension)

\textbf{Beobachung} Wenn \(f: V\to W \) Isomorphismus $\implies f^{-1}: W\to V$ ist auch ein Isomorphismus.
...
\textbf{Lemma} sei \(f\) wie oben. Sei \(U \subseteq \mKer(f)\) ein Untervektorraum mit \(U \cap \mKer(f) = \{0\}\).  Dann ist \(f|_U:U\to f(U)\) ein Isomorphismus.
...
Lemma. Es gilt: \todo{Stimmt das?}\(u = \sum_{i=1}^k \lambda_ie_i \), weil \(e_1, \dots e_k\) eine Basis im Kern ist, und \(u = \sum_{i=k+1}^{n} \) weil \(u\in U\). Also: \(\sum_{i=1}^k \lambda_ie_i - \sum_{i=k+1}^{n} \lambda_ie_i = 0 \xRightarrow{e_1,\dots, e_n \text{Basis}} \text{alle }\lambda_i = 0 \implies u =0\).

Das Lemma sagt jetzt \(f|_U\) ist ein Isomorphismus. Ausserdem gilt f"ur \(v =\sum_{i=1}^{n}\lambda_ie_i \in V:\)
\begin{align*}
