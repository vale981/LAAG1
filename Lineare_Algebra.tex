...
\begin{itemize}
	\item[] f injektiv
	\item[\(\iff\)] \(\mKer(f) = \{0\} \)
	\item[\(\iff\)] \(\mRg(f) = \dim V\)
\end{itemize}
Andererseits gilt:
\begin{itemize}
	\item[] f surjektiv
	\item[\(\iff\)] \(\mIm(f) = W \)
	\item[\(\iff\)] \(\mRg(f) = \dim W\)
\end{itemize}
\end{proposition}
...
Wenn \(V\) ein Vektorraum ist, \(B,B'\) zwei Basen, dann erhalten wir die
 Basiswechselmatrix \(S\), deren Spalten die Koordinaten von den neuen Basisvektoren \((b_1', \dots, b_n')\) bez"uglich der alten Basis \(B = (b_1, \dots, b_n)\) sind. Es folgt sofort aus den Definitionen: \(S=M^{B'}_B(\mId_V)\)
\subsection{Schlagworte:}
