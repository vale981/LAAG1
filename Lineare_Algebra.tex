...
\begin{theo}{}{}
Sei \(f:V\rightarrow W\) eine lineare Abbildung, sei \(V\) endlichdimensional. Dann gilt:
\[\dim \:\text{Ker}(f) + \dim\: \text{Im}(f) = \dim\: V \]
\end{theo}

\textbf{Intuitives Verständnis:} dim Ker(f) = „verlorene“ Dimensionen, dim Im(f) = „übriggebliebene“ Dimensionen.

\begin{lemma}
	 Sei \(f:V\rightarrow W\), V endlichdimensional. Sei \(U \subseteq V\) mit \(U\cap \text{Ker}(f)=\{0\}\) Dann ist die Einschränkung von f auf U \(f|_U: U\rightarrow f(U) \) ein Isomorphismus.
\end{lemma}

\begin{prof}[des Lemmas] \label{}
$f|_U: U\rightarrow f(U)$ ist surjektiv nach Konstruktion.\\ Zu zeigen: $f|_U$ injektiv $\iff \text{Ker}(f|_U)=\{0\}$\\ Sei $u \in \ker(f|_U)$. Dann gilt: $u\in U \land u\in \ker(f) \iff u\in U \cap \ker(f) = \{0\} \implies u=0 $ 
\end{prof}

\begin{prof}[der Dimensionsformel] \label{}
W"ahle eine Basis \(\{e_1, ..., e_k\}\) in $\ker(f)$ und ergänze sie zu einer Basis \(\{e_1, ..., e_n\}\) in \(V\).

Betrachte nun den Untervektorraum \(U:=\langle e_{k+1}, ..., e_n\rangle \subseteq V\). Es gilt: $U \cap \ker(f)=\{0\}$, weil für $u\in U \cap \ker(f)$ gilt:\\
\(u=\sum\limits_{i=1}^{k}\lambda_i e_i \), weil $\{e_1,\dots,e_k\}$ Basis im Kern ist, und \(u=\sum\limits_{i=k+1}^{n}\lambda_i e_i \), weil $u\in U$.\\
\(\implies \sum\limits_{i=1}^{k}-\sum\limits_{i=k+1}^{n} = 0 \implies \lambda_i=0 \quad \forall i \implies u=0 \)

Das Lemma besagt jetzt, dass $f|_U: U\rightarrow f(U)$ ein Isomorphismus ist.\\
Außerdem gilt für v=$\sum\limits_{i=1}^{n}\lambda_i e_i \in V:$
\[f(v) = f(\sum\limits_{i=1}^{k}\lambda_i e_i) + f(\sum\limits_{i=k+1}^{n}\lambda_i e_i) = f(\sum\limits_{i=k+1}^{n}\lambda_i e_i)\]
